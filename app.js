...
function pushSharedData() {
    ...
    }, 80); // fast push so changes appear everywhere quickly
}

// Live sync every 1s — chat/DM appear on other phones quickly
if (typeof window !== 'undefined') {
    ...
    setInterval(async () => {
        ...
        if (currentUser) {
            ...
            if (fresh) {
                currentUser = fresh;
                if (fresh.banned || fresh.role === 'declined') {
                    forceOutToLogin(fresh.banned ? 'banned' : 'declined', fresh.banReason || '');
                    return;
                }
                if (isMemberTimedOut(fresh)) showTimeoutScreen(fresh);
                else hideTimeoutScreen();
            }
        }
        // Live update chat / DM without wiping the input box
        if (currentPage === 'chat' && typeof window._drawChat === 'function') {
            if (chatMessages.length !== prevLen || chatMessages.length !== _lastChatLen) {
                window._drawChat();
                _lastChatLen = chatMessages.length;
            }
        }
        if (currentPage === 'dm' && typeof window._drawDM === 'function') {
            window._drawDM();
        }
        if (currentPage === 'pending' && canManagePending()) {
            /* pending counts can change — optional soft refresh left to manual */
        }
    }, 1000);
}
...
function saveChat(list) {
    ...
    try { localStorage.setItem('fm_last_save', new Date().toISOString()); } catch (e) {}
    pushSharedDataNow(); // messages reach other phones ASAP
}

function pushSharedDataNow() {
    if (!SHARED_MODE) return;
    clearTimeout(_pushTimer);
    try {
        fetch('/api/data', {
...
function getNavForRole(role) {
    ...
    if (role === 'administration') {
        return all.filter(i => ['dashboard','chat','dm','members','roles','rules','pending','shop','extras','profile'].includes(i.page));
    }
    if (role === 'loan-shark') {
        return all.filter(i => ['dashboard','chat','dm','debts','shop','rules','extras','profile','members'].includes(i.page));
    }
    if (role === 'mathematician') {
        return all.filter(i => ['dashboard','chat','dm','math','rules','extras','profile','members'].includes(i.page));
    }
    if (role === 'og') {
        return all.filter(i => ['dashboard','chat','dm','shop','rules','extras','profile','members','roles'].includes(i.page));
    }
    if (role === 'dept-owners') {
        return all.filter(i => ['dashboard','chat','dm','members','shop','rules','extras','profile'].includes(i.page));
    }
    if (role === 'member') {
        return all.filter(i => ['dashboard','chat','dm','debts','shop','rules','extras','profile'].includes(i.page));
    }
...
function startSplash() {
    ...
    // Sequential: Loading X… then Loaded X… (real steps, not spam)
    const steps = [
        'engine modules',
        'Fry Menu core',
        'member database',
        'chat services',
        'loan shark protocols',
        'role permissions',
        'Friday countdown',
        'shop inventory',
        'security layer',
        'notification system',
        'media handlers',
        'Ford sponsorship pack',
        'music radio',
        'analytics engine',
        'network connection'
    ];

    // Ford logo + free CC video (Big Buck Bunny — open movie, not a Ford ad)
    const content = document.querySelector('.splash-content');
    if (content && !document.getElementById('splash-ford-img')) {
        const logoWrap = document.querySelector('.ford-logo');
        if (logoWrap) {
            logoWrap.innerHTML = `
                <img id="splash-ford-img" src="/assets/ford-logo.svg" alt="Ford" style="width:min(280px,70vw);height:auto;display:block;margin:0 auto 8px;">
                <span class="tm">™</span>
            `;
        }
        const bg = document.createElement('div');
        bg.id = 'splash-drive-bg';
        bg.style.cssText = 'position:absolute;inset:0;z-index:0;overflow:hidden;pointer-events:none;background:#000;';
        bg.innerHTML = `
            ...
            <div style="position:absolute;inset:0;background:radial-gradient(ellipse at center,transparent 20%,#000 75%);"></div>
        `;
        splash.insertBefore(bg, splash.firstChild);
    ...
    let step = 0;
    let phase = 'loading'; // loading | loaded
    const stepMs = 900; // each half-step lasts ~0.9s so it feels real
    const totalSteps = steps.length * 2; // loading + loaded per item
    const loadMs = totalSteps * stepMs + 1200;
    ...
    tickStatus();
    const statusTimer = setInterval(tickStatus, stepMs);
...
function forceOutToLogin(kind, reason) {
    clearSession();
    currentUser = null;
    hideTimeoutScreen();
    appEl.classList.add('hidden');
    ...
    const msg = kind === 'banned'
        ? ('You are banned.' + (reason ? ' Reason: ' + reason : ''))
        : 'Your account was declined. Contact the owner.';
    notify(kind === 'banned' ? 'Banned' : 'Declined', msg, 'danger');
    initMusicPlayer();
}

function showTimeoutScreen(m) {
    let el = document.getElementById('timeout-screen');
    if (!el) {
        el = document.createElement('div');
        el.id = 'timeout-screen';
        el.className = 'timeout-screen';
        document.body.appendChild(el);
    }
    el.classList.remove('hidden');
    const until = Number(m.timeoutUntil || 0);
    const reason = m.timeoutReason || 'No reason given';
    function paint() {
        ...
        el.innerHTML = `
            <div class="timeout-card">
                ...
                <h2>You are timed out</h2>
                <p class="text-muted">You cannot use most features until this ends.</p>
                <div class="timeout-cd">${String(h).padStart(2,'0')}:${String(min).padStart(2,'0')}:${String(s).padStart(2,'0')}</div>
                <p style="margin-top:12px;"><strong>Why:</strong> ${escapeHtml(reason)}</p>
                <button class="btn btn-secondary" style="margin-top:16px;" onclick="logout()">Back to login</button>
            </div>
        `;
        if (left <= 0) {
            hideTimeoutScreen();
            m.timeoutUntil = 0;
            saveMembers(members);
            notify('Timeout over', 'You can use the app again', 'success');
        }
    }
    paint();
    if (window._timeoutPaint) clearInterval(window._timeoutPaint);
    window._timeoutPaint = setInterval(paint, 1000);
}

function hideTimeoutScreen() {
    const el = document.getElementById('timeout-screen');
...
const FRY_PLAYLIST = [
    ...
    { title: 'Inner Light', artist: 'Elderbrook, Bob Moses', id: '40tPP3K10yMZxwnT65REKj' },
...
function showApp() {
    if (currentUser && (currentUser.banned || currentUser.role === 'declined')) {
        forceOutToLogin(currentUser.banned ? 'banned' : 'declined', currentUser.banReason || '');
        return;
    }
    ...
    initMusicPlayer();
    if (currentUser && isMemberTimedOut(currentUser)) {
        showTimeoutScreen(currentUser);
    } else {
        hideTimeoutScreen();
    }
}
...
async function renderPage(page) {
    ...
    const titles = {
        ...
        dm: 'Direct Messages', extras: 'Extras', settings: 'Settings', profile: 'My Profile'
    };
    ...
    switch (page) {
        ...
        case 'extras': renderExtras(); break;
...
function renderChat() {
    ...
    function drawMessages() {
        ...
        box.innerHTML = generalMsgs().map(m => {
            ...
            return `
            <div class="wa-row ${mine ? 'wa-mine' : 'wa-other'}">
                <div class="wa-bubble ${mine ? 'wa-bubble-mine' : 'wa-bubble-other'}">
                    ${!mine ? `<div class="wa-name">${escapeHtml(m.name)}</div>` : ''}
                    <div class="wa-body">${mediaHtml(m)}</div>
                    <div class="wa-time">
                        ${m.time || ''}
                        ${mine ? ' ✓✓' : ''}
                        ${mine ? ` <button type="button" class="msg-del" data-id="${m.id}">Delete</button>` : ''}
                    </div>
                </div>
            </div>`;
        }).join('') || `<div class="text-muted text-sm" style="text-align:center;padding:24px;">No messages yet. Say hello 👋</div>`;
    ...
    if (canSend && chatAllowed) {
        ...
        document.getElementById('chat-video').addEventListener('change', e => {
            const file = e.target.files && e.target.files[0];
            if (!file) return;
            fileToDataUrl(file, 500 * 1024 * 1024, data => {
                pushMsg({ type: 'video', text: '[Video]', data });
                notify('Video sent', 'Large video — may take a moment on other phones', 'success');
            });
            e.target.value = '';
        });
...
function renderDM() {
    ...
    if (peer) {
        ...
        function draw() {
            const live = chatMessages.filter(m => m.channel === channel);
            const atBottom = box.scrollHeight - box.scrollTop - box.clientHeight < 80;
            box.innerHTML = live.map(m => {
                ...
                <div class="wa-row ${mine ? 'wa-mine' : 'wa-other'}">
                    <div class="wa-bubble ${mine ? 'wa-bubble-mine' : 'wa-bubble-other'}">
                        <div class="wa-body">${escapeHtml(m.text || '')}</div>
                        <div class="wa-time">${m.time || ''}${mine ? ' ✓✓' : ''}
                        ${mine ? ` <button type="button" class="msg-del" data-id="${m.id}">Delete</button>` : ''}
                        </div>
                    </div>
                </div>`;
        ...
        window._drawDM = draw;
        function send() {
            if (isMemberMuted(currentUser) || isMemberTimedOut(currentUser)) {
                notify('Blocked', 'Muted or timed out', 'danger');
                return;
            }
                                ...
                                <td>
                                    ${m.role !== 'owner' ? `
                                        <div class="flex gap-2" style="flex-wrap:wrap;">
                                            ...
                                            ${isMemberMuted(m)
                                                ? `<button class="btn btn-success btn-sm" onclick="unmuteMember(${m.id})">Unmute</button>`
                                                : `<button class="btn btn-secondary btn-sm" onclick="muteMember(${m.id})">Mute</button>`}
                                            ${isMemberTimedOut(m)
                                                ? `<button class="btn btn-success btn-sm" onclick="untimeoutMember(${m.id})">Un-timeout</button>`
...
function untimeoutMember(id) {
    ...
    m.timeoutReason = '';
    saveMembers(members);
    postAnnouncement(`${m.name} timeout cleared`);
    notify('Un-timeout', m.name + ' can use the app again', 'success');
    renderPage('members');
}
...
function renderDebts() {
    ...
    const hasActiveLoan = myOwed.length > 0;
    ...
    content.innerHTML = `
        ...
        <div class="card mb-4">
            <p class="text-sm text-muted">No active loans — payback countdown is hidden. Borrow to start one.</p>
        </div>`}
...
function renderExtras() {
    const tips = [
        'Pay loans before Monday 09:00 to avoid R2/day interest.',
        'Use DMs for private talks — General is for everyone.',
        'Mathematician can see all Friday take-outs.',
        'Loan Shark must Accept loans before they become owed.',
        'Timeout shows a full-screen countdown with the reason.',
        'Banned users are kicked back to the login screen.',
        'Tap 🎵 for Fry Menu Radio (Spotify — full tracks).',
        'Owner can Mute / Timeout / Ban from Members.'
    ];
    const tip = tips[Math.floor(Math.random() * tips.length)];
    const poll = JSON.parse(localStorage.getItem('fm_poll_v1') || 'null');
    const reactions = JSON.parse(localStorage.getItem('fm_reactions_v1') || '{}');
    const mood = localStorage.getItem('fm_mood_v1') || '';
    const notes = localStorage.getItem('fm_personal_notes_v1') || '';
    const luck = Math.floor(Math.random() * 100) + 1;

    content.innerHTML = `
        <div class="card mb-4">
            <h3 class="card-title">✨ Extras hub</h3>
            <p class="text-sm text-muted">Tools, games, and fun — data stays on this device + shared server.</p>
        </div>

        <div class="card mb-4">
            <h3 class="card-title">💡 Random tip</h3>
            <p>${escapeHtml(tip)}</p>
            <button class="btn btn-secondary btn-sm" onclick="renderExtras()">New tip</button>
        </div>

        <div class="card mb-4">
            <h3 class="card-title">🎲 Luck roll</h3>
            <p style="font-size:2rem;font-weight:700;color:var(--accent);">${luck}</p>
            <button class="btn btn-primary btn-sm" onclick="renderExtras()">Roll again</button>
        </div>

        <div class="card mb-4">
            <h3 class="card-title">😊 Mood check-in</h3>
            <div class="flex gap-2" style="flex-wrap:wrap;">
                ${['🔥','😎','😴','😤','🥳','💰'].map(e =>
                    `<button class="btn btn-secondary btn-sm" onclick="setMood('${e}')">${e}</button>`
                ).join('')}
            </div>
            <p class="text-sm text-muted" style="margin-top:8px;">Your mood: ${mood || 'not set'}</p>
        </div>

        <div class="card mb-4">
            <h3 class="card-title">📝 Personal notes</h3>
            <textarea class="form-control" id="personal-notes" rows="4" placeholder="Only on this phone…">${escapeHtml(notes)}</textarea>
            <button class="btn btn-primary btn-sm" style="margin-top:8px;" onclick="savePersonalNotes()">Save notes</button>
        </div>

        <div class="card mb-4">
            <h3 class="card-title">📊 Quick poll</h3>
            ${poll ? `
                <p><strong>${escapeHtml(poll.q)}</strong></p>
                ${(poll.options || []).map((o,i) => `
                    <button class="btn btn-secondary btn-sm" style="margin:4px;" onclick="votePoll(${i})">${escapeHtml(o)} (${(poll.votes&&poll.votes[i])||0})</button>
                `).join('')}
            ` : `<p class="text-sm text-muted">No poll yet. Owner can create one in Settings.</p>`}
        </div>

        <div class="card mb-4">
            <h3 class="card-title">🏆 Friday leaderboard</h3>
            <div class="table-wrap">
                <table>
                    <thead><tr><th>#</th><th>Name</th><th>Friday take-out</th></tr></thead>
                    <tbody>
                        ${members.filter(m => !['declined'].includes(m.role) && !m.banned)
                            .slice().sort((a,b) => Number(b.fridayTakeout||0) - Number(a.fridayTakeout||0))
                            .slice(0,10).map((m,i) => `
                            <tr><td>${i+1}</td><td>${escapeHtml(m.name)}</td><td>${formatRand(m.fridayTakeout)}</td></tr>
                        `).join('')}
                    </tbody>
                </table>
            </div>
        ...
        <div class="card mb-4">
            <h3 class="card-title">🎯 Who’s online vibe</h3>
            <p class="text-sm">${members.filter(m => !m.banned && m.role !== 'declined').length} active accounts · ${members.filter(m => m.role==='pending').length} pending</p>
            <button class="btn btn-secondary btn-sm" onclick="notify('Ping','You sent a vibe check 👋','success')">Send vibe check</button>
        </div>

        <div class="card mb-4">
            <h3 class="card-title">🧠 Quick calc</h3>
            <input class="form-control" id="extra-calc" placeholder="e.g. 20+15*2">
            <button class="btn btn-primary btn-sm" style="margin-top:8px;" onclick="runExtraCalc()">Calculate</button>
            <p id="extra-calc-out" class="text-sm" style="margin-top:8px;"></p>
        </div>

        <div class="card mb-4">
            <h3 class="card-title">📦 Data safety</h3>
            <p class="text-sm text-muted">Updates keep your data — members, chat, loans stay on the server + backup.</p>
            <button class="btn btn-secondary btn-sm" onclick="backupAllData();notify('Backup','Saved','success')">Backup now</button>
        </div>
    `;
}

function setMood(e) {
    localStorage.setItem('fm_mood_v1', e);
    notify('Mood', e, 'success');
    renderExtras();
}
function savePersonalNotes() {
    const v = document.getElementById('personal-notes')?.value || '';
    localStorage.setItem('fm_personal_notes_v1', v);
    notify('Saved', 'Notes stored on this phone', 'success');
}
function votePoll(i) {
    const poll = JSON.parse(localStorage.getItem('fm_poll_v1') || 'null');
    if (!poll) return;
    poll.votes = poll.votes || poll.options.map(() => 0);
    poll.votes[i] = (poll.votes[i] || 0) + 1;
    localStorage.setItem('fm_poll_v1', JSON.stringify(poll));
    pushSharedData();
    notify('Voted', poll.options[i], 'success');
    renderExtras();
}
function runExtraCalc() {
    const expr = document.getElementById('extra-calc')?.value || '';
    const out = document.getElementById('extra-calc-out');
    try {
        if (!/^[\d.+\-*/()\s]+$/.test(expr)) throw new Error('bad');
        const val = Function('"use strict"; return (' + expr + ')')();
        if (out) out.textContent = '= ' + val;
    } catch {
        if (out) out.textContent = 'Invalid expression';
    }
}
...
window.unmuteMember = unmuteMember;
window.untimeoutMember = untimeoutMember;
window.pushSharedDataNow = pushSharedDataNow;
window.confirmTimeout = confirmTimeout;
window.banMember = banMember;
window.forceOutToLogin = forceOutToLogin;
window.showTimeoutScreen = showTimeoutScreen;
window.hideTimeoutScreen = hideTimeoutScreen;
window.renderExtras = renderExtras;
window.setMood = setMood;
window.savePersonalNotes = savePersonalNotes;
window.votePoll = votePoll;
window.runExtraCalc = runExtraCalc;
