...
// Live sync every 2s so all phones stay in sync
if (typeof window !== 'undefined') {
    setInterval(async () => {
        ...
        if (fresh) {
            ...
            if (isMemberTimedOut(fresh)) {
                showTimeoutScreen(fresh);
            } else {
                hideTimeoutScreen();
            }
        }
        // soft refresh current page data without full redraw spam
        if (currentPage === 'chat' || currentPage === 'pending' || currentPage === 'members' || currentPage === 'debts') {
            // light touch — only re-render if still on app
            try {
...
function startSplash() {
    ...
    // Setup splash visuals: Ford logo image + ambient bg (no copyrighted Ford commercial)
    ...
    if (content && !document.getElementById('splash-ford-img')) {
        ...
        // Ambient gradient “drive” animation instead of copyrighted video
        const bg = document.createElement('div');
        bg.id = 'splash-drive-bg';
        bg.style.cssText = 'position:absolute;inset:0;z-index:0;overflow:hidden;pointer-events:none;';
        bg.innerHTML = `<div style="position:absolute;inset:-20%;background:radial-gradient(ellipse at 30% 40%,#003a6b 0%,#000 55%),linear-gradient(120deg,transparent 40%,rgba(0,80,160,.25) 50%,transparent 60%);animation:fordDrive 12s linear infinite;"></div>
        <style>@keyframes fordDrive{0%{transform:translateX(-8%) scale(1.05)}100%{transform:translateX(8%) scale(1.05)}}</style>`;
...
function renderChat() {
    ...
    if (canSend && chatAllowed) {
        ...
        document.getElementById('chat-video').addEventListener('change', e => {
            const file = e.target.files && e.target.files[0];
            if (!file) return;
            // Large video allowance (browser memory still limits real size; 5000MB is not practical in a web app)
            fileToDataUrl(file, 200 * 1024 * 1024, data => {
                pushMsg({ type: 'video', text: '[Video]', data });
                notify('Video sent', 'Very large videos may fail on some phones', 'success');
            });
            e.target.value = '';
        });
